...
#(
    ...
    parameter int N_BNKS = 2
) (
    //Input signals from AGE Group
    input  logic [ N_AGE-1:0][NBIT_ADDR-1:0] age_addr_i,
    input  logic [ N_AGE-1:0][   N_BNKS-1:0] age_bank_i,
    input  logic [ N_AGE-1:0]                age_valid_i,
    input  logic [ N_AGE-1:0]                age_we_i,
  ...
  generate
    for (genvar i = 0; i < N_AGE; i++) begin : gen_we
  ...
  always_comb begin
    dmem_bank = '0;
    for (int j = 0; j < N_AGE; j = j + 1) begin
