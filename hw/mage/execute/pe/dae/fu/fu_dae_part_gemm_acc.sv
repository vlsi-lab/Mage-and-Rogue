...
(
  ...
  assign is_add_instr = (instr_i == SUB || instr_i == ADD || instr_i == ACC);

  assign adder_op_b_negate = (instr_i == SUB);

  // prepare operand a
  assign adder_op_a = is_add_instr ? ((instr_i == ACC) ? op_a : a_i) : '0;
  ...
  always_comb begin
    ...
    end else if (instr_i == ADD || instr_i == ACC) begin
  ...
  // actual adder
  assign adder_result_expanded  = (instr_i == ADD || instr_i == SUB || instr_i == ACC || (instr_i == MUL && no_vec_mode)) ? ($signed(
      adder_in_a
  ) + $signed(
      adder_in_b
  )) : '0;
